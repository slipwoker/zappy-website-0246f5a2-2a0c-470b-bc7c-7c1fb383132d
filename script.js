...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (phoneHeaderBtn) {
    phoneHeaderBtn.addEventListener('click', function() {
      const phoneNumber = '+972546242000';
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
        ...
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '0246f5a2-2a0c-470b-bc7c-7c1fb383132d',
                name: name,
                email: email,
                subject: 'Contact Form Submission - ' + (company || 'No Company'),
      ...
      alert('תודה ' + name + '! ההודעה נשלחה בהצלחה. נחזור אליך בהקדם.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "אורי ניב - Ori Niv Original Winner Product משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
